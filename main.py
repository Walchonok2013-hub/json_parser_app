...
class JSONParserApp:
    ...
    def setup_ui(self):
        ...
        list_frame.grid(row=2, column=0, pady=10, sticky=(tk.W, tk.E, tk.N, tk.S), ipady=5)
        ...
        form_frame.grid(row=3, column=0, pady=10, sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        self.id_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=2, padx=(5, 20))
        ...
        self.name_entry.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=2, padx=(5, 20))


        ttk.Label(form_frame, text="Description:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.desc_entry = ttk.Entry(form_frame, textvariable=self.desc_var)
        self.desc_entry.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=2, padx=(5, 20))

        ttk.Label(form_frame, text="Price:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.price_entry = ttk.Entry(form_frame, textvariable=self.price_var)
        self.price_entry.grid(row=3, column=1, sticky=(tk.W, tk.E), pady=2, padx=(5, 20))

        # Frame для кнопок формы
        ...
        form_frame.columnconfigure(1, weight=1)  # для растягивания полей ввода
    ...
    def refresh_list(self):
        try:
            ...
            if response.status_code == 200:
                ...
                messagebox.showerror("Ошибка API", f"Ошибка загрузки данных: {response.status_code}")
    ...
    def clear_form(self):
        ...
        # Отключаем кнопки Update и Delete
    ...
    def create_item(self):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                messagebox.showerror("Ошибка создания", f"Не удалось создать элемент: {error_text}")
        except ValueError:
    ...
    def update_item(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                messagebox.showwarning("Элемент не найден", "Элемент с указанным ID не существует")
            else:
                error_text = response.json().get('error', 'Неизвестная ошибка')
                messagebox.showerror("Ошибка обновления", f"Не удалось обновить элемент: {error_text}")
        except ValueError:
    ...
    def delete_item(self):
        ...
        if not item_id:
            messagebox.showwarning("Ошибка", "Пожалуйста, выберите элемент для удаления")
            return
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                error_text = response.json().get('error', 'Неизвестная ошибка')
                messagebox.showerror("Ошибка удаления", f"Не удалось удалить элемент: {error_text}")
        except requests.exceptions.RequestException as e:
            messagebox.showerror("Ошибка соединения", f"Не удалось подключиться к API:\n{str(e)}")
